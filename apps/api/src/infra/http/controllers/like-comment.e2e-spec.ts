...
describe('Like Comment (E2E)', () => {
  ...
  test('[POST] /comments/:commentId/like', async () => {
  ...
  test('[POST] /comments/:commentId/like — should not be able to like the same comment twice', async () => {
    const user = await userFactory.makePrismaUser()
    const accessToken = jwtService.sign({ sub: user.id.toString() })
  ...
  test('[POST] /comments/:commentId/like — should not be able to like a non-existent comment', async () => {
    const user = await userFactory.makePrismaUser()
    const accessToken = jwtService.sign({ sub: user.id.toString() })

    const response = await request(app.getHttpServer())
      .post('/comments/non-existent-comment-id/like')
      .set('Authorization', `Bearer ${accessToken}`)

    expect(response.statusCode).toBe(404)
  })

  test('[POST] /comments/:commentId/like — should not be able to like without authentication', async () => {
    const user = await userFactory.makePrismaUser()
