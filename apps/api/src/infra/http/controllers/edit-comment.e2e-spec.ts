...
describe('Edit Comment (E2E)', () => {
  ...
  test('[PATCH] /comments/:commentId', async () => {
    ...
    const response = await request(app.getHttpServer())
      .patch(`/comments/${comment.id.toString()}`)
  ...
  test('[PATCH] /comments/:commentId — should not be able to edit another user comment', async () => {
  ...
  test('[PATCH] /comments/:commentId — should not be able to edit a non-existent comment', async () => {
    const user = await userFactory.makePrismaUser()
    const accessToken = jwtService.sign({ sub: user.id.toString() })

    const response = await request(app.getHttpServer())
      .patch('/comments/non-existent-comment-id')
      .set('Authorization', `Bearer ${accessToken}`)
  ...
  test('[PATCH] /comments/:commentId — should not be able to edit without authentication', async () => {
    const user = await userFactory.makePrismaUser()
    ...
    const response = await request(app.getHttpServer())
      .patch(`/comments/${comment.id.toString()}`)
      .send({ content: 'Unauthenticated edit.' })
